...
const findMissingNumberSolution4 = (arr, n) => {
  // Initialize two variables to store XOR results
  let xor1 = 0;
  let xor2 = 0;
  ...
  for (let index = 0; index < arr.length; index++) {
    ...
    xor1 = xor1 ^ arr[index];
    // XOR all indices + 1 (to match the range 1 to N-1)
    xor2 = xor2 ^ (index + 1);
  }

  // XOR the last number N to complete the range 1 to N
  xor2 = xor2 ^ n;

  // The missing number is found by XORing the two results
  // xor1 contains XOR of all elements in the array
  // xor2 contains XOR of all numbers from 1 to N
  // Since XOR of two same numbers is 0, all paired numbers will cancel out
